...
;writecommand
	...
	;LDR R1,=GPIO_PORTA_DATA_R	;3) Clear D/C=PA6 to zero
	;LDR R2, [R1]
	;BIC	R2, #0x40
	;STR R2, [R1]
...
;commandloop
	...
	;AND R2, #0x10
	;CMP R2, #0x10
	;BEQ commandloop				;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
...
;writedata
	...
	;LDR R2, [R1]
	;AND R2, #0x02
	;CMP R2, #0x00				;2) If bit 1 is low loop back to step 1 (wait for TNF bit to be high)
	;BEQ	writedata
	;LDR R1,=GPIO_PORTA_DATA_R	;3) Set D/C=PA6 to 1
	;LDR R2, [R1]
	;ORR R2, #0x40
	;STR R2, [R1]
